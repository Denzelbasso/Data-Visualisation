...
def df_fixer_vimp(df, n, exi, column_retriever):
    ...
    return df
##### DATA PREPARATION ################################################################################################
##Preparation of the information needed for the 2 dataframes:
#Value imported by country
...
##Creation of dataframe df that contains Import and Export value for top 5 countries (+world)
...
dfs = []
#Add in df the info in the dataframed created before (the country)
...
## Creation of the 2nd dataframe for export by top country
def graph2(country, df):
    ...
    for i in range(1, 20):
        ...
        df2.reset_index()
        df2best = pd.concat([df2best, df2], ignore_index=True)
...
##### APP #####################################################################################################
## Creation of the image that does not need interactions
##### IMAGE 1 - World map
def map():
...
app.layout = html.Div([
    html.Div([
        ...
        html.Div([
            html.P('The goal of this dashboard is to develop an interactive visualisation of data about the exportation of wine. The dashboard will enable the user to analyse the top 5 European exporters in depth. It’s divided into 2 parts:',
    ...
    html.Div([
        ...
        html.Div([
            html.H2(
            children='Graph 2. - Top 6 countries that the selected country exports the most wine to, in thousands of euros',
...
,
    ...
    html.Div([
        ...
        html.Div([
            html.H2(children='Image 1. - World map of value exported wine by country, in thousands of euros',
                    style={'font-family': 'Times New Roman'}),
            dcc.Graph(id='map',
                      figure=map()
                      ),
...
######Creation of the Interactive graph and filters########

#### GRAPH 1 - Evolution of the value of export and import of wine of the selected country, in thousands of euros
#Filter 1 : years and country
@app.callback(
...
#### GRAPH 2 - Top 6 countries that the selected country exports the most wine to, in thousands of euros
#Filter 2 - Country
@app.callback(
...
### GRAPH 3 - Evolution of the value of export of wine of the selected countries between 2003 to 2022, in thousands of euros
#Filter 3 - Country year
@app.callback(
